% =========================================================================
% Lab-03 Simulation: Square Wave with Various Noise Types
% 
% This script demonstrates the effect of different types and amplitudes of
% noise on a clean square wave signal.
%
% It will:
% 1. Generate a clean square wave.
...
outputDir = 'Square_Wave_Plots';
if ~exist(outputDir, 'dir') % Check if the folder doesn't already exist
    mkdir(outputDir);       % Create the folder
end

% --- 1. Generate the Clean Square Wave ---
fprintf('Step 1: Generating the clean square wave...\n');

duty = 50;       % 50% duty cycle for the square wave
f = 4*10^9;      % Frequency (using the same as the lab for consistency)
ti = 1/f;         % Time period
s = ti/100;       % Sampling period
t = 0:s:8*ti;     % Time vector for 8 periods

% Generate the clean square wave (amplitude is +/- 1)
x_square = square(2*pi*f*t, duty);

% --- Plot and save the original clean signal ---
figure('Name', 'Clean Square Wave', 'NumberTitle', 'off');
plot(t, x_square, 'k', 'LineWidth', 1.5);
title('Original Clean Signal (Square Wave)');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;
ylim([-4 4]); % Set a consistent Y-axis limit for all plots for easy comparison
saveas(gcf, fullfile(outputDir, 'Square_0_Clean_Signal.jpg'));
...
a1 = 0.1; b1 = -0.1;
n1 = (b1-a1) .* rand(size(x_square)) + a1;
xn1 = x_square + n1;
...
subplot(2,1,2);
plot(t, xn1);
title('Noisy Signal (xn1): Square wave is clearly visible');
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Square_1_Low_Uniform_Noise.jpg'));

% --- Case 2: Moderate Uniform Noise [-0.5, 0.5] ---
fprintf('Step 2.2: Processing Case 2 - Moderate Noise...\n');
a2 = 0.5; b2 = -0.5;
n2 = (b2-a2) .* rand(size(x_square)) + a2;
xn2 = x_square + n2;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Square_2_Moderate_Uniform_Noise.jpg'));

% --- Case 3: High Uniform Noise (Signal = Noise) [-1, 1] ---
fprintf('Step 2.3: Processing Case 3 - High Noise...\n');
a3 = 1.0; b3 = -1.0;
n3 = (b3-a3) .* rand(size(x_square)) + a3;
xn3 = x_square + n3;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Square_4_Very_High_Uniform_Noise.jpg'));

% --- Case 5: High Gaussian Noise ---
fprintf('Step 2.5: Processing Case 5 - High Gaussian Noise...\n');
n5 = 1.0 * randn(size(x_square)); % Gaussian noise with standard deviation 1.0
xn5 = x_square + n5;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(output_directory, 'Square_5_High_Gaussian_Noise.jpg'));

% --- Case 6: Positive-Only Uniform Noise [0, 1] ---
fprintf('Step 2.6: Processing Case 6 - Positive-Only Noise...\n');
a6 = 1.0; b6 = 0;
n6 = (b6-a6) .* rand(size(x_square)) + a6;
xn6 = x_square + n6;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Square_6_Positive_Uniform_Noise.jpg'));

% --- Case 7: Negative-Only Uniform Noise [-1, 0] ---
fprintf('Step 2.7: Processing Case 7 - Negative-Only Noise...\n');
a7 = 0; b7 = -1.0;
n7 = (b7-a7) .* rand(size(x_square)) + a7;
xn7 = x_square + n7;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Square_7_Negative_Uniform_Noise.jpg'));
