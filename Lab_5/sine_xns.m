% =========================================================================
% Lab-03 Simulation: Sine Wave with Various Noise Types
% 
% This script demonstrates the effect of different types and amplitudes of
% noise on a clean sine wave signal.
%
% It will:
...
outputDir = 'Sine_Wave_Plots';
if ~exist(outputDir, 'dir') % Check if the folder doesn't already exist
    mkdir(outputDir);       % Create the folder
end

% --- 1. Generate the Clean Sine Wave ---
fprintf('Step 1: Generating the clean sine wave...\n');
...
% Generate the clean sine wave (amplitude is 1)
x_sine = sin(2*pi*f*t);

% --- Plot and save the original clean signal ---
figure('Name', 'Clean Sine Wave', 'NumberTitle', 'off');
plot(t, x_sine, 'k', 'LineWidth', 1.5);
title('Original Clean Signal (Sine Wave)');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;
ylim([-4 4]); % Set a consistent Y-axis limit for all plots for easy comparison
saveas(gcf, fullfile(outputDir, 'Sine_0_Clean_Signal.jpg'));
...
subplot(2,1,2);
plot(t, xn1);
title('Noisy Signal (xn1): Sine wave is clearly visible');
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Sine_1_Low_Uniform_Noise.jpg'));

% --- Case 2: Moderate Uniform Noise [-0.5, 0.5] ---
fprintf('Step 2.2: Processing Case 2 - Moderate Noise...\n');
a2 = 0.5; b2 = -0.5;
n2 = (b2-a2) .* rand(size(x_sine)) + a2;
xn2 = x_sine + n2;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Sine_2_Moderate_Uniform_Noise.jpg'));

% --- Case 3: High Uniform Noise (Signal = Noise) [-1, 1] ---
fprintf('Step 2.3: Processing Case 3 - High Noise...\n');
a3 = 1.0; b3 = -1.0;
n3 = (b3-a3) .* rand(size(x_sine)) + a3;
xn3 = x_sine + n3;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Sine_4_Very_High_Uniform_Noise.jpg'));

% --- Case 5: High Gaussian Noise ---
fprintf('Step 2.5: Processing Case 5 - High Gaussian Noise...\n');
n5 = 1.0 * randn(size(x_sine)); % Gaussian noise with standard deviation 1.0
xn5 = x_sine + n5;
...
xlabel('Time (s)'); ylabel('Amplitude'); grid on; ylim([-4 4]);
saveas(gcf, fullfile(outputDir, 'Sine_7_Negative_Uniform_Noise.jpg'));
